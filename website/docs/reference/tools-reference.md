...
These tools are thin Hermes adapters around a local Docker sandbox stack. They are registered only when `/home/lucky/docker/untrusted-link-sandbox` (or `HERMES_UNTRUSTED_LINK_SANDBOX_DIR`) has `docker-compose.yml` and the expected executable wrappers under `bin/`: `triage`, `audit-url`, `audit-url-cdp`, `audit-repo`, and `inspect-download`.
